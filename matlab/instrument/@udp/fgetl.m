...
% Error checking.
if length(obj) > 1
    error(message('instrument:fgetl:invalidOBJ'));
end

if nargout > 5
    error(message('instrument:fgetl:invalidSyntax'));
end
...
try
    ...
    newExc = MException('instrument:fgetl:opfailed',aException.message );
    throw(newExc);
end
...
if ~isempty(warningstr)
    ...
    if nargout < 3
        warning('instrument:fgetl:unsuccessfulRead', warningstr);
    end
...
varargout = {dataValues, dataCount, warningstr, datagramAddress, datagramPort};
end