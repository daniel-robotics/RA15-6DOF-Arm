...
if strncmpi(get(obj, 'DatagramTerminateMode'), 'on', 2) == 1
    readCompleteDg = true;
else
...
if numRead ~= totalSize && ~readCompleteDg
    ...
    varargout = {data, numRead, warningstr, datagramAddress, datagramPort};
...
try
    ...
    if length(data) == size(1)*size(2)
        dataValues = reshape(data, size(1), size(2));
    else
        dataValues = data;
    end
    ...
    dataValues = double(data);
    outputMessage = getString(message('instrument:fscanf:dataReshapeFailed'));
...
if ~isempty(outputMessage)
    ...
    if ~exceptionCaught
        ...
        dispWarning = message('instrument:fread:unsuccessfulRead', outputMessage);
    else
        dispWarning = outputMessage;
    end
    if nargout < 3
       warning(dispWarning);
    end
...
varargout = {dataValues, numRead, outputMessage, datagramAddress, datagramPort};
% --------------------------------------------------------------
