...
if ~isa(obj, 'icinterface')
    error(message('instrument:binblockwrite:invalidOBJInterface'));
end

if length(obj)>1
    error(message('instrument:binblockwrite:invalidOBJDim'));
end
...
switch nargin
case 1
   error(message('instrument:binblockwrite:invalidSyntaxA'));
case 2
   cmd          = varargin{1};
   precision    = 'uchar';
   header       = '';
...
otherwise
   error(message('instrument:binblockwrite:invalidSyntaxArgv'));
end   

% Error checking.
if ~isa(precision, 'char')
	error(message('instrument:binblockwrite:invalidPRECISIONstring'));
end
if ~isa(headerformat, 'char')
	error(message('instrument:binblockwrite:invalidHEADERFORMATstring'));
end
...
function out = localParseString(format, cmd)
...
if ~isempty(errmsg)
    newExc = MException('instrument:binblockwrite:invalidHEADERFORMAT', 'Invalid HEADERFORMAT specified. Type ''instrhelp binblockwrite'' for more information.');
