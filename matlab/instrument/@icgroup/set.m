...
% Error if invalid.
if ~all(isvalid(obj))
   error(message('instrument:icgroup:set:invalidOBJ'));
end
...
if (nargout == 0)
   ...
   if nargin == 1
      if (length(obj) == 1)
         ...
         error(message('instrument:icgroup:set:nolhswithvector'));
      end
   else
      % Ex. set(obj, 'BaudRate');
      ...
      try
         ...
         if (nargin == 2)
            if ischar(varargin{1}) 
                ...
                if (length(obj) > 1)
                    error(message('instrument:icgroup:set:scalarHandle'));
                end
            ...
            elseif isstruct(varargin{1})
                ...
                error(message('instrument:icgroup:set:invalidPVPair'));
            end
         else
            % Ex. set(obj, 'BaudRate', 4800); 
            tempObj = igetfield(obj, 'jobject');
   ...
   try
	  ...
	  case 1
          % Ex. out = set(obj);
          if (length(obj) > 1)
              error(message('instrument:icgroup:set:scalarHandleLength'));
          end        
