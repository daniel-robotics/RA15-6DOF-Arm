...
% Error checking.
if ~isa(obj, 'icgroup')
    error(message('instrument:icgroup:propinfo:invalidOBJ'));
end
 
if (length(obj) > 1)
   error(message('instrument:icgroup:propinfo:invalidOBJDim'));
end
...
case 1
    ...
    try
        ...
        newExc = MException('instrument:icgroup:propinfo:opfailed',aException.message);
        throw(newExc);
    end
...
case 2
    ...
    if ~(ischar(prop) || iscellstr(prop))
        error(message('instrument:icgroup:propinfo:invalidPROPERTY'));
    end
