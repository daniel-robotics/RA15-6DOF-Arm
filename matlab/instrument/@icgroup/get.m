...
% Error if invalid.
if ~all(isvalid(obj))
   error(message('instrument:icgroup:get:invalidOBJ'));
end

if  ((nargout == 0) && (nargin == 1))
   % Ex. get(obj)
   if (length(obj) > 1)
      error(message('instrument:icgroup:get:nolhswithvector'))
   else
      try
          localCreateGetDisplay(obj);
      catch aException
          newExc = MException('instrument:icgroup:get:opfailed', aException.message);
          throw(newExc);  
      end
      return;
   end
...
elseif (nargin > 2)
    error(message('instrument:icgroup:get:maxrhs'));
else
   % Ex. get(obj, 'BaudRate')
   try      
      ...
      if ~(ischar(property) || iscellstr(property))
          error(message('instrument:icgroup:get:invalidArg'));
      end
