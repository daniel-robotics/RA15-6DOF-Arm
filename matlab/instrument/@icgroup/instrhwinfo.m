...
%             Interface:      Adaptor:
%             ==========      ======== 
%             gpib            advantech, agilent, cec, contec, ics, iotech, 
%                             keithley, mcc, ni
%             visa            agilent, ni, tek
%             i2c             aardvark, ni845x 
%             spi             aardvark
...
% Error checking.
if ~isa(obj, 'icgroup')
    error(message('instrument:icgroup:instrhwinfo:invalidOBJDevice'));
end

% Error if invalid.
if ~all(isvalid(obj))
   error(message('instrument:icgroup:instrhwinfo:invalidOBJ'));
end
...
case 2
    ...
    if ~(ischar(prop) || iscellstr(prop))
        error(message('instrument:icgroup:instrhwinfo:invalidFieldName'));
    end
    ...
    for i=1:length(obj)
        ...
        try
            ...
            newExc = MException('instrument:icgroup:instrhwinfo:opfailed',aException.message );
            throw(newExc);
        end
        out(i,:) = cell(tempOut)';
    end
