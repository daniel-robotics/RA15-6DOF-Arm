...
%SAVEOBJ Save filter for device group objects.
%
%   B = SAVEOBJ(OBJ) is called by SAVE when a device group object
%   is saved to a .MAT file. The return value B is subsequently used 
%   by SAVE to populate the .MAT file.  
%
...
savedObj = isetfield(savedObj, 'store', propInfo);
...
for i = 1:length(parseProps)
    ...
    switch className
    case {'serial', 'gpib', 'visa', 'udp', 'tcpip', 'icdevice'}
