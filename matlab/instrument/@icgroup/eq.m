...
%EQ Overload of == for device group objects.
%
...
if isempty(arg1)
    if (length(arg2) == 1)
       ...
       error(message('instrument:icgroup:eq:dimagree'));
   end
   return;
elseif isempty(arg2)
   if (length(arg1) == 1)
...
% Determine if both objects are output objects.
try   
   ...
   iseq = (jarg1 == jarg2);
    ...
    if strcmp(aException.identifier, 'instrument:icgroup:eq:dimagree')
        rethrow(aException);
    end
