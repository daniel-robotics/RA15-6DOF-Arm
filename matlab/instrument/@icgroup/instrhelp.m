...
% Error checking.
if nargin > 2
    error(message('instrument:icgroup:instrhelp:invalidSyntaxArgv'));
end

if nargout > 1
	error(message('instrument:icgroup:instrhelp:invalidSyntaxRet'))
end

if ~isa(obj, 'icgroup')
	error(message('instrument:icgroup:instrhelp:invalidArgFirst'));
end

if ~any(isvalid(obj))
    error(message('instrument:icgroup:instrhelp:invalidOBJ'))
end
...
% Error checking.
if ~ischar(name)
    error(message('instrument:icgroup:instrhelp:invalidArgSecond'));
end
...
case 1
	...
	switch nargout
	case 0
        out1 = evalc('help([instrRoot filesep ''icgroup'' filesep ''icgroup''])'); 
        disp(out1);
        for i=1:length(out)
            fprintf(out{i})
        end
	case 1
		varargout{1} = [help([instrRoot filesep 'icgroup' filesep 'icgroup']) out{:}];
	end
	return;
case 2
    ...
    if errflag
        if (~isempty(strmatch(name, methods(obj))))
            ...
            switch nargin
                case 1
                    error(message('instrument:icgroup:instrhelp:invalidNAME', varargin{ 1 }));
                case 2
                    fpVar = sprintf('%s.%s',class(obj), name);
                    error(message('instrument:icgroup:instrhelp:invalidNAME', fpVar));
            end
        end
    else
        % Create output - either to command line or to output variable.
...
try
    ...
    out = com.mathworks.toolbox.instrument.device.util.PropertyHelp.getHelp(java(jobj(1)), prop);
catch
    errflag = true;
end
