...
for i=1:length(gui_ha)
    name        = gui_ha{i};
    nextItem    = cell(hardwareAssets.get(java.lang.String(name)));
...
for i=1:length(gui_ds)
    name     = gui_ds{i};
    nextItem = driverSessions.get(java.lang.String(name));
...
for i=1:length(gui_ln)
    name          = gui_ln{i};
    nextItem      = cell(logicalNames.get(java.lang.String(name)));
    description   = nextItem{1};
...
while (names.hasMoreElements)
    out{end+1} = char(names.nextElement); %#ok<AGROW>
end

% ---------------------------------------------------------
function localRemoveItemsFromStore(store, guiNames, current, type)

% Nothing to compare against if the config store object property is empty.
if (isempty(current))
    return;
end
