...
if  ((nargout == 0) && (nargin == 1))
   ...
   if (length(obj) > 1)
      ...
      try 
          ...
          newExc =  MException('instrument:get:opfailed',aException.message);
          throw(newExc);
      end
      return;
   end
...
elseif (nargin > 2)
    error(message('instrument:get:maxrhs'));
else
   % Ex. get(obj, 'BaudRate')
   try      
      ...
      if ~(ischar(property) || iscellstr(property))
          error(message('instrument:get:invalidArg'));
      end
...
try
    ...
    newExc = MException('instrument:get:invalidArg','Objects must all be instances of the same class.');
...
try
	if ischar(property)
        ...
        if (~isempty(out) && isnumeric(out) && (numel(out) > 1) && isvector(out))
            r = size(out);
            if r(1) > 1
                out = out';
            end
    ...
    newExc = MException('instrument:get:invalidArg', aException.message);
    throw(newExc);
end
