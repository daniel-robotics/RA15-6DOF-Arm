...
%SAVEOBJ Save filter for instrument objects.
%
%   B = SAVEOBJ(OBJ) is called by SAVE when an instrument object is
...
if ~isempty(objsSaved)
	for i = 1:length(objsSaved)
		if isequal(objsSaved{i}, obj) && ~isa(obj, 'instrument')
			...
			warning(message('instrument:saveobj:recursive'));			
...
instrumentField = igetfield(obj, 'instrument');
propInfo = rmfield(propInfo, 'jobject');
instrumentField = isetfield(instrumentField, 'store', propInfo);
savedObj = isetfield(savedObj, 'instrument', instrumentField);
...
if isa(java(currentObj), 'com.mathworks.toolbox.instrument.device.icdevice.ICDevice')
    settable = localFindSettablePropertiesDevice(currentObj);
elseif isa(java(currentObj), 'com.mathworks.toolbox.instrument.device.icdevice.ICDeviceChild')
...
% Find the non-settable properties.
nonSettableProps = com.mathworks.toolbox.instrument.device.util.PropertyUtil.getNonSettable(jobj);
nonSettableProps(cellfun('isempty', nonSettableProps)) = [];
...
if strcmp(jobj.getDriverTypeDisplayName, 'MATLAB interface object')
%     constructorArgs.object = saveobj(getInterface(jobj));
constructorArgs.object = (getInterface(jobj));
end
...
for j = 0:groups.size-1
    ...
    for i = 1:length(jgroup)
        ...
        % Find the non-settable properties.
        nonSettableProps = com.mathworks.toolbox.instrument.device.util.PropertyUtil.getNonSettable(jgroup(i));
        nonSettableProps(cellfun('isempty', nonSettableProps)) = [];
...
% Find the non-settable properties.
nonSettableProps = com.mathworks.toolbox.instrument.device.util.PropertyUtil.getNonSettable(jparent);
nonSettableProps(cellfun('isempty', nonSettableProps)) = [];
...
% If a MATLAB interface object driver is being used, save the interface object.
if strcmp(jparent.getDriverTypeDisplayName, 'MATLAB interface object')
