...
%
%   OBJ = LOADOBJ(B) is called by LOAD when a I2C object is 
...
for i = 1:length(propInfo)
    ...
    elseif isstruct(propVals)
        ...
        vendor = constructorArgs{1}(1);
        board = constructorArgs{1}(2);
        remoteChip = constructorArgs{1}(3);
        if(strcmpi(vendor,'test'))
            pathToDll='Not_Used';
        else
            pathToDll = localFindAdaptor([vendor 'i2c']);
        end
        propInfo(i).obj = handle(javaObject(['com.mathworks.toolbox.instrument.I2C' upper(vendor)], pathToDll, vendor, board, remoteChip));
...
if exist(adaptorRoot,'file')
    ...
    newExc = MException('instrument:i2c:adaptorNotFound' );
