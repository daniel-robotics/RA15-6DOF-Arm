...
%LOADOBJ Load filter for BlueTooth objects.
%
%   OBJ = LOADOBJ(B) is called by LOAD when a Bluetooth object is
...
for i = 1:length(propInfo)
    ...
    elseif isstruct(propVals)
        ...
        [a b]=strtok(propVals.ConstructorArgs(1),':');
        propInfo(i).obj = handle(javaObject('com.mathworks.toolbox.instrument.Bluetooth', a,uint8(str2double(b(2:end)))));
