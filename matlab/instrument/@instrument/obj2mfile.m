...
%   name as the file containing the instrument object code.  
%
...
%   is read-only.
%
%   To recreate OBJ, type the name of the file.  
%
%   Example:
%       g = gpib('ni', 0, 2);
...
if nargin > 5
	error(message('instrument:obj2mfile:invalidSyntaxArgv'));
end

% Error checking.
if ~isa(objects, 'instrument')
    error(message('instrument:obj2mfile:invalidOBJInstrument'));
end

if ~all(isvalid(objects))
   error(message('instrument:obj2mfile:invalidOBJ'));
end
...
switch nargin
case 0
   error(message('instrument:obj2mfile:invalidSyntaxObj'));
case 1
   error(message('instrument:obj2mfile:invalidSyntaxFile'));
case 2
...
case 3
  ...
  if ~ischar(varargin{1})
	  error(message('instrument:obj2mfile:invalidArg'));
  end
...
if ~ischar(filename)
	error(message('instrument:obj2mfile:invalidFILENAME'));
end
...
if ~ischar(definelist)
   error(message('instrument:obj2mfile:invalidMODE'));
end

if isempty(strmatch(syntax, {'set','dot'}))
   error(message('instrument:obj2mfile:invalidSyntaxdot'));
end
...
if isempty(strmatch(reuse, {'reuse', 'create'}))
   error(message('instrument:obj2mfile:invalidREUSE'));
end
...
if (fid < 3)
   error(message('instrument:obj2mfile:invalidFile', filename));
end
...
structInfo.jobject = objects.jobject;
structInfo.constructor = objects.constructor;
structInfo.store = objects.store;
...
% ------------------------------------------------------------------
% Create the comments for the beginning of the file.
function localSetComment(fid, file, type, name)

fprintf(fid,['%%' upper(file) ' Code for creating an ' type ' object.\n',...
   '%%   \n',...
   '%%   This is the machine generated representation of an ' type ' object.\n',...
   '%%   This file, ' upper(file) '.M, was generated from the OBJ2MFILE function.\n',...
   ...
   '%%   file but with a .MAT extension. To recreate this ', type, ' object,\n',...
   '%%   type the name of the file, ' file ', at the MATLAB command prompt.\n',...
   '%%   \n',...
   '%%   The file, ' upper(file) '.M and its associated MAT-file, ',...
         upper(file) '.MAT (if \n',...
