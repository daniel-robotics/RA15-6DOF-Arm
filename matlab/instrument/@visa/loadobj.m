...
%
%   OBJ = LOADOBJ(B) is called by LOAD when a VISA object is
...
for i = 1:length(propInfo)
    ...
    elseif isstruct(propVals)
        ...
        switch (propVals.Type)
            ...
            case 'visa-pxi'
                propInfo(i).obj = handle(com.mathworks.toolbox.instrument.PxiVisa(constructorArgs{:}));
            case 'visa-gpib-vxi'
            ...
            case 'visa-generic'
                propInfo(i).obj = handle(com.mathworks.toolbox.instrument.GenericVisa(constructorArgs{:}));
        end
...
for i = 1:length(propInfo)
    ...
    for j = 1:length(allLocations)
        ...
        if isnumeric(loc.Index)
            ...
            currentPropValue = localFillInArray(currentPropValue, localWrapIntoInstrument(currentObj), loc.Index); %#ok<*NASGU>
        else
...
if isempty(strfind(className, 'javahandle.com.mathworks.toolbox.instrument.'))
    if isempty(strfind(className, 'Device'))
        obj = uddObj;
        return;
    end
end
...
switch class(uddObj)
    ...
    case 'javahandle.com.mathworks.toolbox.instrument.VxiVisa'
        obj = visa(uddObj);
    case 'javahandle.com.mathworks.toolbox.instrument.PxiVisa'
        obj = visa(uddObj);
    ...
    case 'javahandle.com.mathworks.toolbox.instrument.UsbVisa'
        obj = visa(uddObj);
    case 'javahandle.com.mathworks.toolbox.instrument.GenericVisa'
        obj = visa(uddObj);
