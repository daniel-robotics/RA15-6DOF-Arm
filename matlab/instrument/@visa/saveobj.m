...
% Check to see if object has alread been saved. If saveobj is called
% From ICDEVICE, this object may have already been saved. 
if iscell(obj.jobject)
    out = obj;
    return;
end
...
nonSettableProps(strcmp('Type', nonSettableProps)) = [];
...
function propInfo = localParseUserDataAndFcnProps(~, propInfo)
...
for i = 1:length(parseProps)
    ...
    switch className
    case {'serial', 'Bluetooth', 'gpib', 'visa', 'udp', 'tcpip', 'icdevice', 'icgroup'}
...
% Determine if the object has already been saved.
if strfind(class(currentObj), 'ICGroup')
...
for i = 1:2:length(index)
    ...
    case '()'
        stringToAccessUDD = [stringToAccessUDD '(' num2str(index{i+1}) ')']; %#ok<AGROW>
    case '{}'
        stringToAccessUDD = [stringToAccessUDD '{' num2str(index{i+1}) '}']; %#ok<AGROW>
    case '.'
        stringToAccessUDD = [stringToAccessUDD '.' index{i+1}]; %#ok<AGROW>
    end
end
...
for i = 1:length(propNames)
    if strfind(propNames{i},'Fcn')
        parseProps{count} = propNames{i}; %#ok<AGROW>
        count = count+1;
    end
end
