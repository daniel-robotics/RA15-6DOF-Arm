classdef ErrorInfo < instrument.ivic.IviGroupBase
    ...
    methods
        function [Code,Description] = GetError(obj,BufferSize)
            ...
            %since the user last called IviFgen_GetError or
            %IviFgen_ClearError.
            ...
            try
                ...
                status = calllib( libname,'IviFgen_GetError', session, Code, BufferSize, Description);
        ...
        function ClearError(obj)
            ...
            %have a session handle to pass to the IviFgen_GetError
            %function, which occurs when a call to IviFgen_init or
            %IviFgen_InitWithOptions fails.
            
            narginchk(1,1)
            try
                [libname, session ] = obj.getLibraryAndSession();
                
                status = calllib( libname,'IviFgen_ClearError', session);
