classdef UserOptions < instrument.ivic.IviGroupBase
    ...
    properties
        ...
        %Use the IviDCPwr_InitWithOptions function to override this
        %value.
        Range_Check
        ...
        %only when the IVIDCPWR_ATTR_SIMULATE attribute is set to
        ...
        %ViInt32 and ViReal64 attributes.  If IVIDCPWR_ATTR_SPY is
        %set to VI_TRUE, you use the NI-Spy utility to view coercion
        %information.  If IVIDCPWR_ATTR_SPY is set to VI_FALSE you
        %call IviDCPwr_GetNextCoercionRecord to extract and delete
        %the oldest coercion record from the list.     The default
        %value is VI_FALSE.   Use the IVI Configuration utility or
        %the IviDCPwr_InitWithOptions function to override this
        %value.
        ...
        %warnings when you call the IviDCPwr_Initiate and
        %IviDCPwr_Measure functions.  The default value is VI_FALSE.
        % Interchangeability warnings indicate that using your
        %application with a different instrument might cause
        %different behavior.  If IVIDCPWR_ATTR_SPY is set to
        %VI_FALSE, you call IviDCPwr_GetNextInterchangeWarning to
        %extract interchange warnings.  Call the
        %IviDCPwr_ClearInterchangeWarnings function to clear the
        %list of interchangeability warnings without reading them.
        %If IVIDCPWR_ATTR_SPY is set to VI_TRUE, you can use the NI
        %Spy utility to view interchangeability warnings.
        ...
        %have not set that attribute.  (2) The IviDCPwr class
        %defines values for an attribute and you have set that
        %attribute to a value not defined by the IviDCPwr class.
        %(3) The IviDCPwr class defines an attribute as read-only
        %and you have set that attribute.
        Interchange_Check
        ...
        %function calls.     The default value is VI_TRUE.
        Spy
    end
