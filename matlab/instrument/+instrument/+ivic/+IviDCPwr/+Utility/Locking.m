classdef Locking < instrument.ivic.IviGroupBase
    ...
    methods
        function CallerHasLock = LockSession(obj)
            ...
            %application called IviDCPwr_LockSession.  - A call to the
            %instrument driver locked the session.  - A call to the IVI
            %engine locked the session.  After your call to
            %IviDCPwr_LockSession returns successfully, no other threads
            %can access the instrument session until you call
            %IviDCPwr_UnlockSession.  Use IviDCPwr_LockSession and
            %IviDCPwr_UnlockSession around a sequence of calls to
            ...
            %IviDCPwr_LockSession within the same thread.  To completely
            %unlock the session, you must balance each call to
            %IviDCPwr_LockSession with a call to IviDCPwr_UnlockSession.
            % If, however, you use the Caller Has Lock parameter in all
            %calls to IviDCPwr_LockSession and IviDCPwr_UnlockSession
            %within a function, the IVI Library locks the session only
            %once within the function regardless of the number of calls
            %you make to IviDCPwr_LockSession.  This allows you to call
            %IviDCPwr_UnlockSession just once at the end of the
            %function.
            ...
            try
                ...
                status = calllib( libname,'IviDCPwr_LockSession', session, CallerHasLock);
        ...
        function CallerHasLock = UnlockSession(obj)
            ...
            %IviDCPwr_LockSession.  Refer to IviDCPwr_LockSession for
            ...
            try
                ...
                status = calllib( libname,'IviDCPwr_UnlockSession', session, CallerHasLock);
