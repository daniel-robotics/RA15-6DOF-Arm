classdef Utility < instrument.ivic.IviGroupBase
    ...
    methods (Hidden=true)
        function obj = Utility()
            ...
            obj.ErrorInfo = instrument.ivic.IviScope.Utility.ErrorInfo();
            obj.InterchangeabilityInfo = instrument.ivic.IviScope.Utility.InterchangeabilityInfo();
            obj.CoercionInfo = instrument.ivic.IviScope.Utility.CoercionInfo();
            obj.Locking = instrument.ivic.IviScope.Utility.Locking();
        end
    ...
    methods
        function reset(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_reset', session);
        ...
        function ResetWithDefaults(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_ResetWithDefaults', session);
        ...
        function [SelfTestResult,SelfTestMessage] = self_test(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_self_test', session, SelfTestResult, SelfTestMessage);
        ...
        function Disable(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_Disable', session);
        ...
        function InvalidateAllAttributes(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_InvalidateAllAttributes', session);
        ...
        function [InstrumentDriverRevision,FirmwareRevision] = revision_query(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_revision_query', session, InstrumentDriverRevision, FirmwareRevision);
        ...
        function [ErrorCode,ErrorMessage] = error_query(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_error_query', session, ErrorCode, ErrorMessage);
        ...
        function ErrorMessage = error_message(obj,ErrorCode)
            ...
            try
                ...
                status = calllib( libname,'IviScope_error_message', session, ErrorCode, ErrorMessage);
        ...
        function ChannelString = GetChannelName(obj,Index,BufferSize)
            %GETCHANNELNAME The function returns the channel string
            %that is in the channel table at an index you specify.
            ...
            try
                [libname, session ] = obj.getLibraryAndSession();
                ChannelString = libpointer('int8Ptr', repmat(10, 1, 4096 ));
                
                status = calllib( libname,'IviScope_GetChannelName', session, Index, BufferSize, ChannelString);
        ...
        function SpecificDriverCHandle = GetSpecificDriverCHandle(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_GetSpecificDriverCHandle', session, SpecificDriverCHandle);
        ...
        function IsInvalid = IsInvalidWfmElement(obj,ElementValue)
            %ISINVALIDWFMELEMENT After the read and fetch waveform
            %functions execute, each element in the waveform array
            %contains either a voltage or a value indicating that the
            %oscilloscope could not sample a voltage.  The driver uses
            %an IEEE defined NaN (Not a Number) value to mark as invalid
            %each element in the waveform array for which the
            %oscilloscope could not sample a voltage.  This function
            %determines whether a value you pass from the waveform array
            %is invalid.
            
            narginchk(2,2)
            ElementValue = obj.checkScalarDoubleArg(ElementValue);
            try
                [libname, session ] = obj.getLibraryAndSession();
                IsInvalid = libpointer('uint16Ptr', 0);
                
                status = calllib( libname,'IviScope_IsInvalidWfmElement', session, ElementValue, IsInvalid);
