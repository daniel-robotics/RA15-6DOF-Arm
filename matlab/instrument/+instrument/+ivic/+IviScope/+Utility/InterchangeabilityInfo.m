classdef InterchangeabilityInfo < instrument.ivic.IviGroupBase
    ...
    methods
        function InterchangeWarning = GetNextInterchangeWarning(obj,BufferSize)
            ...
            %different behavior.  If the IVISCOPE_ATTR_SPY attribute is
            %set to False, you use this function to retrieve
            %interchangeability warnings.  If the IVISCOPE_ATTR_SPY
            ...
            %IviScope_InitiateAcquisition () IviScope_ReadWaveform ()
            %IviScope_ReadMinMaxWaveform ()
            %IviScope_ReadWaveformMeasurement ()  If the next
            ...
            try
                ...
                status = calllib( libname,'IviScope_GetNextInterchangeWarning', session, BufferSize, InterchangeWarning);
        ...
        function ClearInterchangeWarnings(obj)
            ...
            try
                ...
                status = calllib( libname,'IviScope_ClearInterchangeWarnings', session);
        ...
        function ResetInterchangeCheck(obj)
            ...
            %guarantee that the IviScope_GetNextInterchangeWarning
            ...
            %calling the IviScope_GetNextInterchangeWarning function
            %until no more interchangeability warnings are returned.  If
            %you are not interested in the content of those warnings,
            %you can call the IviScope_ClearInterchangeWarnings function.
            
            narginchk(1,1)
            try
                [libname, session ] = obj.getLibraryAndSession();
                
                status = calllib( libname,'IviScope_ResetInterchangeCheck', session);
