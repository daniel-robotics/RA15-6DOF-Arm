classdef Locking < instrument.ivic.IviGroupBase
    ...
    methods
        function CallerHasLock = LockSession(obj)
            ...
            %application called IviScope_LockSession.  - A call to the
            %instrument driver locked the session.  - A call to the IVI
            %engine locked the session.  After your call to
            %IviScope_LockSession returns successfully, no other threads
            %can access the instrument session until you call
            %IviScope_UnlockSession.  Use IviScope_LockSession and
            %IviScope_UnlockSession around a sequence of calls to
            ...
            %IviScope_LockSession within the same thread.  To completely
            %unlock the session, you must balance each call to
            %IviScope_LockSession with a call to IviScope_UnlockSession.
            % If, however, you use the Caller Has Lock parameter in all
            %calls to IviScope_LockSession and IviScope_UnlockSession
            %within a function, the IVI Library locks the session only
            %once within the function regardless of the number of calls
            %you make to IviScope_LockSession.  This allows you to call
            %IviScope_UnlockSession just once at the end of the
            %function.
            ...
            try
                ...
                status = calllib( libname,'IviScope_LockSession', session, CallerHasLock);
        ...
        function CallerHasLock = UnlockSession(obj)
            ...
            %IviScope_LockSession.  Refer to IviScope_LockSession for
            ...
            try
                ...
                status = calllib( libname,'IviScope_UnlockSession', session, CallerHasLock);
