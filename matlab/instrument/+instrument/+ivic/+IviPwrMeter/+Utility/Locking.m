classdef Locking < instrument.ivic.IviGroupBase
    ...
    methods
        function CallerHasLock = LockSession(obj)
            ...
            %application called IviPwrMeter_LockSession.  - A call to
            %the instrument driver locked the session.  - A call to the
            %IVI engine locked the session.  After your call to
            %IviPwrMeter_LockSession returns successfully, no other
            %threads can access the instrument session until you call
            %IviPwrMeter_UnlockSession.  Use IviPwrMeter_LockSession and
            %IviPwrMeter_UnlockSession around a sequence of calls to
            ...
            %IviPwrMeter_LockSession within the same thread.  To
            %completely unlock the session, you must balance each call
            %to IviPwrMeter_LockSession with a call to
            %IviPwrMeter_UnlockSession.  If, however, you use the Caller
            %Has Lock parameter in all calls to IviPwrMeter_LockSession
            %and IviPwrMeter_UnlockSession within a function, the IVI
            %Library locks the session only once within the function
            %regardless of the number of calls you make to
            %IviPwrMeter_LockSession.  This allows you to call
            %IviPwrMeter_UnlockSession just once at the end of the
            %function.
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_LockSession', session, CallerHasLock);
        ...
        function CallerHasLock = UnlockSession(obj)
            ...
            %IviPwrMeter_LockSession.  Refer to IviPwrMeter_LockSession
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_UnlockSession', session, CallerHasLock);
