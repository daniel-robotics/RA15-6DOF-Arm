classdef Error < instrument.ivic.IviGroupBase
    ...
    methods
        function [Code,Description] = GetError(obj,BufferSize)
            ...
            %since the user last called IviPwrMeter_GetError or
            %IviPwrMeter_ClearError.
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_GetError', session, Code, BufferSize, Description);
        ...
        function ClearError(obj)
            ...
            %valid IVI session for the "instrument handle" parameter,
            %this function clears the error information for the session.
            %If the user passes VI_NULL for the Vi parameter, this
            %function clears the error information for the current
            %execution thread. If the Vi parameter is an invalid
            %session, the function does nothing and returns an error.
            %The function clears the error code by setting it to
            %VI_SUCCESS.  If the error description string is non-NULL,
            %the function de-allocates the error description string and
            %sets the address to VI_NULL.  Maintaining the error
            %information separately for each thread is useful if the
            %user does not have a session handle to pass to the
            %IviPwrMeter_GetError function, which occurs when a call to
            %IviPwrMeter_init or IviPwrMeter_InitWithOptions fails.
            
            narginchk(1,1)
            try
                [libname, session ] = obj.getLibraryAndSession();
                
                status = calllib( libname,'IviPwrMeter_ClearError', session);
