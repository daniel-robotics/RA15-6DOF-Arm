    ...
    methods
        function AttributeValue = GetAttributeViInt32(obj,ChannelName,AttributeID)
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_GetAttributeViInt32', session, ChannelName, AttributeID, AttributeValue);
        ...
        function AttributeValue = GetAttributeViReal64(obj,ChannelName,AttributeID)
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_GetAttributeViReal64', session, ChannelName, AttributeID, AttributeValue);
        ...
        function AttributeValue = GetAttributeViString(obj,ChannelName,AttributeID,BufferSize)
            ...
            %parameter.  If the current value of the attribute,
            %including the terminating NUL byte, is larger than the size
            %you indicate in the Buffer Size parameter, the function
            %copies Buffer Size-1 bytes into the buffer, places an ASCII
            %NUL byte at the end of the buffer, and returns the buffer
            %size you must pass to get the entire value.  For example,
            %if the value is "123456" and the Buffer Size is 4, the
            %function places "123" into the buffer and returns 7.  If
            %you want to call this function just to get the required
            %buffer size, you can pass 0 for the Buffer Size and VI_NULL
            %for the Attribute Value buffer.    If you want the function
            %to fill in the buffer regardless of the number of bytes in
            %the value, pass a negative number for the Buffer Size
            %parameter.
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_GetAttributeViString', session, ChannelName, AttributeID, BufferSize, AttributeValue);
        ...
        function AttributeValue = GetAttributeViBoolean(obj,ChannelName,AttributeID)
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_GetAttributeViBoolean', session, ChannelName, AttributeID, AttributeValue);
        ...
        function AttributeValue = GetAttributeViSession(obj,ChannelName,AttributeID)
            ...
            try
                ...
                status = calllib( libname,'IviPwrMeter_GetAttributeViSession', session, ChannelName, AttributeID, session);
