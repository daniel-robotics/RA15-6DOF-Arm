classdef InterchangeabilityInfo < instrument.ivic.IviGroupBase
    ...
    methods
        function InterchangeWarning = GetNextInterchangeWarning(obj,BufferSize)
            ...
            %different behavior.  If the IVIDMM_ATTR_SPY attribute is
            %set to False, you use this function to retrieve
            %interchangeability warnings.  If the IVIDMM_ATTR_SPY
            ...
            %IVIDMM_ATTR_INTERCHANGE_CHECK attribute is set to True and
            %you call one of the following functions:  IviDmm_Initiate
            %() IviDmm_Read () IviDmm_ReadMultiPoint ()  If the next
            ...
            %get the entire value.  For example, if the value is
            %"123456" and the Buffer Size is 4, the function places
            %"123" into the buffer and returns 7.  If you pass a
            %negative number for the buffer size, the function copies
            %the value to the buffer regardless of the number of bytes
            %in the value.  If you pass 0 for the buffer size, you can
            %pass VI_NULL for the Interchange Warning buffer parameter.
            %The function returns an empty string in the Interchange
            %Warning parameter if no interchangeability warnings remain
            %for the session.  Interchangeability checking examines the
            ...
            try
                ...
                status = calllib( libname,'IviDmm_GetNextInterchangeWarning', session, BufferSize, InterchangeWarning);
        ...
        function ClearInterchangeWarnings(obj)
            ...
            %IVIDMM_ATTR_INTERCHANGE_CHECK attribute is set to True.
            ...
            try
                ...
                status = calllib( libname,'IviDmm_ClearInterchangeWarnings', session);
        ...
        function ResetInterchangeCheck(obj)
            ...
            %guarantee that the IviDmm_GetNextInterchangeWarning
            ...
            %calling the IviDmm_GetNextInterchangeWarning function until
            %no more interchangeability warnings are returned.  If you
            %are not interested in the content of those warnings, you
            %can call the IviDmm_ClearInterchangeWarnings function.
            
            narginchk(1,1)
            try
                [libname, session ] = obj.getLibraryAndSession();
                
                status = calllib( libname,'IviDmm_ResetInterchangeCheck', session);
