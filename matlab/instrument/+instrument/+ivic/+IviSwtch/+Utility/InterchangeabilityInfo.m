classdef InterchangeabilityInfo < instrument.ivic.IviGroupBase
    ...
    methods
        function InterchangeWarning = GetNextInterchangeWarning(obj,BufferSize)
            ...
            %different behavior.  If the IVISWTCH_ATTR_SPY attribute is
            %set to False, you use this function to retrieve
            %interchangeability warnings.  If the IVISWTCH_ATTR_SPY
            ...
            %and you call one of the following functions:
            %IviSwtch_Connect () IviSwtch_SetPath ()
            %IviSwtch_InitiateScan ()  If the next interchangeability
            %warning string, including the terminating NUL byte,
            ...
            %and returns 7.  If you pass a negative number for the
            %buffer size, the function copies the value to the buffer
            %regardless of the number of bytes in the value.  If you
            %pass 0 for the buffer size, you can pass VI_NULL for the
            %Interchange Warning buffer parameter.  The function returns
            %an empty string in the Interchange Warning parameter if no
            ...
            %one attribute within that group.  In general, the class
            %driver generates interchangeability warnings when it
            %encounters one of the following conditions:  (1) An
            %attribute that affects the behavior of the instrument is in
            %a state that you did not specify.  (2) You set a
            %class-defined attribute to an instrument-specific value.
            %(3) You set the value of an attribute that the class driver
            %defines as read-only.  (4) The class driver encounters an
            %error when it tries to apply a value to an extension
            %attribute that your program never configures.
            ...
            try
                ...
                status = calllib( libname,'IviSwtch_GetNextInterchangeWarning', session, BufferSize, InterchangeWarning);
        ...
        function ClearInterchangeWarnings(obj)
            ...
            try
                ...
                status = calllib( libname,'IviSwtch_ClearInterchangeWarnings', session);
        ...
        function ResetInterchangeCheck(obj)
            ...
            %guarantee that the IviSwtch_GetNextInterchangeWarning
            ...
            %calling the IviSwtch_GetNextInterchangeWarning function
            %until no more interchangeability warnings are returned.  If
            %you are not interested in the content of those warnings,
            %you can call the IviSwtch_ClearInterchangeWarnings function.
            
            narginchk(1,1)
            try
                [libname, session ] = obj.getLibraryAndSession();
                
                status = calllib( libname,'IviSwtch_ResetInterchangeCheck', session);
