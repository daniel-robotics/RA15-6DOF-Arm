classdef Locking < instrument.ivic.IviGroupBase
    ...
    methods
        function CallerHasLock = LockSession(obj)
            ...
            %application called IviRFSigGen_LockSession.  - A call to
            %the instrument driver locked the session.  - A call to the
            %IVI engine locked the session.  After your call to
            %IviRFSigGen_LockSession returns successfully, no other
            %threads can access the instrument session until you call
            %IviRFSigGen_UnlockSession.  Use IviRFSigGen_LockSession and
            %IviRFSigGen_UnlockSession around a sequence of calls to
            ...
            %IviRFSigGen_LockSession within the same thread.  To
            %completely unlock the session, you must balance each call
            %to IviRFSigGen_LockSession with a call to
            %IviRFSigGen_UnlockSession.  If, however, you use the Caller
            %Has Lock parameter in all calls to IviRFSigGen_LockSession
            %and IviRFSigGen_UnlockSession within a function, the IVI
            %Library locks the session only once within the function
            %regardless of the number of calls you make to
            %IviRFSigGen_LockSession.  This allows you to call
            %IviRFSigGen_UnlockSession just once at the end of the
            %function.
            ...
            try
                ...
                status = calllib( libname,'IviRFSigGen_LockSession', session, CallerHasLock);
        ...
        function CallerHasLock = UnlockSession(obj)
            ...
            %IviRFSigGen_LockSession.  Refer to IviRFSigGen_LockSession
            ...
            try
                ...
                status = calllib( libname,'IviRFSigGen_UnlockSession', session, CallerHasLock);
