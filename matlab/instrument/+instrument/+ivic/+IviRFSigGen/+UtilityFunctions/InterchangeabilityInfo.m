classdef InterchangeabilityInfo < instrument.ivic.IviGroupBase
    ...
    methods
        function InterchangeWarning = GetNextInterchangeWarning(obj,BufferSize)
            ...
            %IVIRFSIGGEN_ATTR_INTERCHANGE_CHECK attribute is set to
            %True.  The function returns an empty string in the
            %Interchange Warning parameter if no interchangeability
            %warnings remain for the session.  In general, the
            %instrument driver generates interchangeability warnings
            %when an attribute that affects the behavior of the
            %instrument is in a state that you did not specify.
            ...
            try
                ...
                status = calllib( libname,'IviRFSigGen_GetNextInterchangeWarning', session, BufferSize, InterchangeWarning);
        ...
        function ClearInterchangeWarnings(obj)
            ...
            try
                ...
                status = calllib( libname,'IviRFSigGen_ClearInterchangeWarnings', session);
        ...
        function ResetInterchangeCheck(obj)
            ...
            %guarantee that the IviRFSigGen_GetNextInterchangeWarning
            ...
            try
                ...
                status = calllib( libname,'IviRFSigGen_ResetInterchangeCheck', session);
