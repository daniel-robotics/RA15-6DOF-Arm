...
%#codegen
    ...
    % validate transport
    transport = string(varargin{1});
    transport = validatestring(transport,instrument.interface.modbus.Modbus.Transports,'modbus','transport');
    
    % create the modbus object
    fh = str2func(sprintf('instrument.interface.modbus.%s.Modbus', transport));
    obj = fh(varargin{:});
