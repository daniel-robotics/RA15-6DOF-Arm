...
%
%   OUT = SPOLL(OBJ) performs a serial poll on the GPIB object, where OBJ
%   an array of GPIB objects. OUT contains the GPIB objects that are ready
%   for servicing. If there are no objects ready for servicing, then OUT is
%   empty.
%
%   If OBJ is an array of GPIB objects, each GPIB object in the array must
%   have the same BoardIndex property value.
%
%   Serial polling is a method for obtaining specific information from GPIB
%   objects when they request service. When you perform a serial poll, OUT
%   contains the GPIB objects that have asserted their SRQ line.
%
%   OUT = SPOLL(OBJ, VAL), where VAL is a numeric array of indices into
%   OBJ, performs a serial poll on the GPIB object, OBJ, and waits until
%   the specified instruments, VAL, have asserted their SRQ line. VAL is a
...
%   OBJ, if VAL is set to 5, it returns an error since OBJ has a length of
%   4.
% 
...
%   addition to the SRQ line status in that second argument.
%
%   Examples:
%       g1 = gpib('ni', 0, 1);
%       g2 = gpib('ni', 0, 2);
...
for m=1:length(objType)
    if ~((strcmp(objType(m),'gpib') || ~isempty(strfind(objType(m), 'visa'))))
        error(message('instrument:spoll:invalidOBJ', 'GPIB', 'GPIB'));
    end
end
...
if length(unique([boardIndex{:}])) ~= 1
    error(message('instrument:spoll:OBJHaveDifferentBoardIndex'));
end
